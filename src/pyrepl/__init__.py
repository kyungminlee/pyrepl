...
class PyREPL:
    ...
    def _print(self, msg: str, target: str = "repl"):
        ...
        for line in plain_text.splitlines(): buffer.append(line)
    ...
    async def _worker(self):
        while True:
            ...
            except Exception as e: self._print(f"Error in fallback: {e}", target="fallback")
    ...
    def _run_dot_command(self, tokens: List[str]):
        ...
        try:
            ...
            stream.flush()
        except Exception as e: self._print(f"Error in command: {e}", target="repl")
        finally: sys.stdout = old_stdout
        ...
        while self.running:
            ...
            for i in range(repl_msg_height):
                ...
                if 0 <= idx < len(self.repl_buffer):
                    try: stdscr.addstr(SEPARATOR_ROW + 1 + i, 0, self.repl_buffer[idx][:self.sw-1])
                    except curses.error: pass
            ...
            if self.mode == REPLMode.INPUT:
                if ch == ord('\n'):
                    ...
                    if line:
                        ...
                        try:
                            ...
                            if tokens[0].startswith('.'): self._run_dot_command(tokens)
                                ...
                                self.pending_queue.append(tokens)
                                self.queue_event.set()
                        except ValueError as e: self._print(f"Tokenization error: {e}", target="repl")
                    ...
                    if len(completion_candidates) == 1:
                        ...
                        self.mode = REPLMode.COMPLETE; completion_select_idx = 0
                ...
                if ch == 9 or ch == curses.KEY_DOWN: completion_select_idx = (completion_select_idx + 1) % len(completion_candidates)
                    ...
                    sel = completion_candidates[completion_select_idx]
                    last_space = current_input.rfind(' ')
                ...
                elif 32 <= ch <= 126: # Typing cancels and adds char
                    self.mode = REPLMode.INPUT; current_input = current_input[:input_pos] + chr(ch) + current_input[input_pos:]; input_pos += 1
                ...
                if ch == ord('\n'):
                    ...
                    except ValueError as e: self._print(f"Edit error: {e}", target="repl")
